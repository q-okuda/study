    ...
    w.addEventListener('load', () => {
      ...
      fetch(url)
        ...
        .then((jsondata) => {
          ...
          // 移動可能範囲設定
          function getMax(a, b) { return Math.max(a, b); };
          function getMin(a, b) { return Math.min(a, b); };
          ...
          function getCoverHeight() {
            ...
            coverWidth = getMax(coverCenter.x - getLinkPos('left').reduce(getMin), getLinkPos('right').reduce(getMax) - coverCenter.x) * 2;
            coverHeight = getMax(coverCenter.y - getLinkPos('top').reduce(getMin), getLinkPos('bottom').reduce(getMax) - coverCenter.y) * 2;
          }
          getCoverHeight();
          ...
          function setCoverSize() {
            ...
            if (coverHeight > containerHeight) {
              cover.style.setProperty('--coverHeight', `${coverHeight + margin * 2}px`);
            } else {
              cover.style.setProperty('--coverHeight', '100%');
            }
            if (coverWidth > containerWidth) {
              cover.style.setProperty('--coverWidth', `${coverWidth + margin * 2}px`);
            } else {
          ...
          // リスト中央配置
          function setCenter() {
            container.scrollLeft = cover.offsetWidth / 2 - container.offsetWidth / 2;
            container.scrollTop = cover.offsetHeight / 2 - container.offsetHeight / 2;
          }
          setCenter();
